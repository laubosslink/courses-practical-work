...
def runAgents( agent, speed, width, timeout, correction ):
    """ The real main. """
    if correction:
        random.seed(0);
    agent.init(width)
...
def readCommand( argv ):
    ...
    parser.add_option('-t', '--timeout', dest='timeout',
                      help = default('Maximum search time (for correction)'), default = 2000)
    parser.add_option('-c', '--correction', action='store_true', dest = 'correction',
                      help = 'Use only for evaluation purpose', default = False)
